...
`re-tlsclient` 是一个基于 `bogdanfinn/tls-client` 的 Go 请求库，目标是更方便地使用 TLS 指纹、HTTP/2 参数与 WebSocket 指纹连接能力。
...
```go
resp, err := client.Get(ctx, "https://httpbin.org/ip", fp.RequestOption{
...
```go
spec, err := fp.ParseGoSpiderSpec(yourGoSpiderSpec)
if err != nil {
    panic(err)
}
...
```go
ws, err := client.NewWebsocket(ctx, "wss://echo.websocket.events", fp.WebsocketOption{
    Headers: map[string]string{
