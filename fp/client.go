...
// ClientOption ...
//   ClientProfile   预置 profile 名称（tls-client 内置的"PascalCase"形式：Okhttp4Android13 / Chrome_131 ...）
//   ExtraProfile    wreq-util 风格小写名（chrome_148 / firefox_151 / safari_26.4 ...），
//                   由 profiles_extra.go 的 ExtraProfiles 解析。ClientProfile 与 ExtraProfile 同时
//                   设置时 ExtraProfile 优先。
//   Spec            TLS_HEX@H1_HEX@H2_HEX 字符串，最高优先级
type ClientOption struct {
	...
	CookieJar                    *jar.Jar     // 自定义CookieJar
	ProxyUrl                     string      // 代理URL
	Closed                       bool        // 是否已关闭
	Debug                        bool        // 是否启用调试模式
	DefaultHeaders               http.Header // 默认请求头
	ClientProfile                string      // 客户端配置文件名称（tls-client 内置 PascalCase）
	ExtraProfile                 string      // wreq-util 风格小写 profile 名（profiles_extra.go 中）
...
var (
	...
	// ProfileMap 是 tls-client 内置 profile 的白名单（仍是 v1.15.1 的子集）。
	// 名称大小写：键小写，值来自 bogdanfinn/tls-client/profiles。
	// 找不到时，NewClient 会回落到 profiles_extra.go 的 ExtraProfiles。
...
func NewClient(ctx g.Ctx, option ...ClientOption) (*Client, error) {
	...
	if len(option) > 0 && option[0].Spec != "" {
		...
		p, sh, sho, psho, err := spec.BuildProfile(option[0].Spec)
		if err != nil {
		...
		prof = profiles.Okhttp4Android12
		if len(option) > 0 && option[0].ClientProfile != "" {
			if p, ok := ProfileMap[option[0].ClientProfile]; ok {
				prof = p
			}
		}
		// ExtraProfile 优先级最高（wreq-util 风格小写名）。
		if len(option) > 0 && option[0].ExtraProfile != "" {
			if p, ok := profile.GetExtraProfile(option[0].ExtraProfile); ok {
				prof = p
			}
		}
	}
...
// DefaultHeaders 返回当前 Client 生效的默认请求头（非 nil 副本）。
// 公开以便 transport 子包复用。
func (c *Client) DefaultHeaders() http.Header {
	return c.defaultHeaders()
}

// ApplyStrictFingerprintHeaders 公开版本，供 transport 子包使用。
func (c *Client) ApplyStrictFingerprintHeaders(header http.Header) {
	c.applyStrictFingerprintHeaders(header)
}

// ApplyHeaderOrdering 公开版本，供 transport 子包使用。
func (c *Client) ApplyHeaderOrdering(header http.Header) {
	c.applyHeaderOrdering(header)
}
...
func (c *Client) doOnce(req *http.Request) (*http.Response, error) {
	...
	case <-timer.C:
		cancel()
		return nil, fmt.Errorf("re-tlsclient: response header timeout after %ds", headerTimeout)
