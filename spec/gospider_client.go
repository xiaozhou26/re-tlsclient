...
// BuildProfile 从 goSpiderSpec 字符串构建 profiles.ClientProfile、默认请求头及其顺序。
// 这是 fp.Client.NewClient 使用的入口；其它包通常用 ParseGoSpiderSpec 解析后访问字段。
func BuildProfile(goSpiderSpec string) (profiles.ClientProfile, http.Header, []string, []string, error) {
	return buildProfileFromSpec(goSpiderSpec)
}
