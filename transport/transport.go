...
// 用法示例:
//
//	client, _ := fp.NewClient(ctx, fp.ClientOption{Spec: "..."})
//	proxy := &httputil.ReverseProxy{
//	    Director:  director,
//	    Transport: transport.New(client),  // 或 transport.New(client, true) 启用 ManageCookies
//	}
...
// New 返回一个实现了 net/http.RoundTripper 的指纹传输层，
// 可用于 httputil.ReverseProxy 等标准库组件。
// manageCookies 为 true 时由本项目的 CookieJar 统一管理 Cookie。
func New(c *fp.Client, manageCookies ...bool) stdhttp.RoundTripper {
	mc := false
...
func (t *FingerprintTransport) RoundTrip(req *stdhttp.Request) (*stdhttp.Response, error) {
	...
	// 严格指纹模式处理
	if t.client.ClientOption.StrictFingerprint {
